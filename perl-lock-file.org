...
second call, which probably isn't expected.
...
the and writing the HTML, and deletes it afterwards. This guarentees that
...
before giving up and killing the program. If the lock file doesn't exist, then
...
characters that File::Temp will fill in to guarentee the file it creates has a
